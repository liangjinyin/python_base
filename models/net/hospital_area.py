...
def baidu_sreach(key, page, client):
    try:
        url = 'https://map.baidu.com/?newmap=1&reqflag=pcmap&biz=1&from=webmap&da_par=direct&pcevaname=pc4.1&qt=s' \
              '&da_src=searchBox.button&wd='+key+' 医院'+'&c=257&src=0&wd2=&pn=0&sug=0&l=12&b=(12571912.199999997,2607450.58;12604616.199999997,2655450.58)&from=webmap&biz_forward={%22scaler%22:2,%22styles%22:%22pl%22}&sug_forward=&auth=IM7xW1028F32Bec%3D9zawUZWDDUI4zCc4uxHHxEVHLxNt1qo6DF%3D%3DCy1uVtcvY1SGpuBtGIiyRWF%3D9Q9K%3DxAwEdwKv7uGccZcuVtPWv3GuBtWykiO%3DUixAC123N5T7XwcEWe1GD8zv7u%40ZPuVteuxxtoqFmqE25524b547I1pt66F9EzeeaCK&device_ratio=2&tn=B_NORMAL_MAP' \
        ...
        sql = 'insert into topl_baidu_temp(uid,name,address,telephone,location,data_type,area_name,di_tag,alias)' \
        ...
        for content in contents:
            ...
            data_list.append(3)  # 4 投资理财，1 银行 ,2 ATM
            data_list.append(content['area_name'])
            data_list.append(content['di_tag'])
            if len(content['ailas'])!=0:
                data_list.append(','.join(content['alias']))
            else:
                data_list.append('')
